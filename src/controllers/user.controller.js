...
export async function sendFriendRequest(req,res) {
    try {
        ...
        if (existingrequest){
            return res.status(400).json({message:"A friend reqquuest already exists between you and this user"})
        }
...
export async function acceptFriendRequest(req,res) {
    try {
        ...
        const friendRequest = await FriendRequest.findOne(rec_id)
        ...
        if(friendRequest.recipient.toString() !== req.user.id){
            return res.status(403).json({message:"Yout are not authorized to accept this request."});
